...
- **P2（资源/体验）**：封面/嵌入图片热路径泄漏、非中文区歌词时间轴、About/Shell 图标句柄释放、封面预览图释放、资源位图缩放释放、清空历史错误可见性已修；仍剩更零散的 Low 级资源释放。
...
| P2-3 Low 级 GDI 释放 | 已实现 | `AboutDialog` 使用 `Icon.ToBitmap()` 后释放源 `Icon`；Shell 文件图标 clone 后释放原 HICON 和 clone；替换封面预览和提取封面时释放临时 `Image`；资源位图缩放后释放 `Graphics` |
