...
- [ ] **StateFieldInstance**（8868 行）：~~`BatchFileTaskContext` 基类（或 `BatchFileProcessor<TItem>` 驱动）统一 ~11 个批处理 context~~ **基类骨架（4 公共字段 cancellationSource/progressDialog/currentFile/processedCount + guarded `Cancel()`）已抽、9 个 batch context 继承 ✅ 完成 `4ec38a2` 2026-07-01（实为 9 个 context 非 ~11;LyricDownload/ReleaseYearSearch 因 progressDialog 是 SimpleProgressDialog + Cancel 语义不同不共享）;各异 `UpdateProgress()` 留派生、runner 循环/取消检查/`TagHistoryRepository` try-finally 脚手架统一待后续**；~~7 个 failure-reporter 类 + `AppendFileError(page,name,msg)`~~ **实为 3 个 `*FailureReporter`（SaveTag/UndoSaveTag/TagSave）、无 `AppendFileError`（scout 误报）;消息优先级核 `ResolveFailureMessage` 已提取 ✅ 完成 `ffbbef87` 2026-07-01（见进度日志）**；~~`PictureCompressionWorker` 的 15 个 `ResizeToNNNQualityMM`/`ResizeToConfiguredLimitQualityMM` 改 `(resolution,quality)[]` 表~~ **✅ 完成 `65b623c` 2026-07-01（见进度日志）**；`CoverPreviewController`（~700 行，4815-5650）/ VirtualMode 选择模型（3711-3816）抽取仍推迟；**`InitializeComponent`（1566 行,原 6864-8429）移入新建 `StateFieldInstance.Designer.cs` partial ✅ 完成 `8aebc2cb` 2026-07-01（主类 internal→internal partial,主文件 8497→6930 行,IL 相同,smoke 启动主窗体验证）**；**`StartClearTags` 完成段三分支消息核 `BuildClearTagsResultMessage` 已提取 ✅ 完成 `ff6cc72f` 2026-07-01（传 Page 引用保 ToString 求值不变,4 characterize + 3-lens 全 EQUIVALENT）**;**`ShowCompletionResult`×2 消息核 `BuildDeleteFilesResultMessage`/`BuildExtractCoversResultMessage` ✅ `b79d07ca`、`ListViewItemNaturalComparer.Compare` 排序纯核 `CompareColumnText` ✅ `b71b6793`、lyrics/comment 截断 `TruncateLyricsOrCommentDisplayValue` ✅ `3b7219a4`（均 characterize + 3-lens 等价）、`IsInvalidRenameFileName`/`ResolveSelectedFilterValue`/`IsNumberedTagFieldValueValid` 谓词核 ✅ `302bfa65`、图片压缩上限 `ResolvePictureCompressionLimits`（int 溢出保留）✅ `c052031`、OnLoad 窗口 clamp 几何 `ClampWindowToWorkingArea`（中风险,数据流/WinForms-setter/无观察者 3-lens 对抗验证纳入）✅ `414275ee`**（均见进度日志）。
...
- [x] **ConfigDescriptorState**：**部分完成 2026-07-01**（`17c096f1` characterize + `bed055a6` 迁移）—— 3 个 raw-field 标识符 switch（`Id3v2FrameId`/`XiphFieldId`/`ApeFieldId`,真正并行的 field→标识符纯映射）合成单张 `rawFieldVocabulary` 表（10 行 × 3 列）+ 3 查表方法（`field != null && TryGetValue` 守卫保持原 `switch(null)→default→null`）。`ReadFieldText`(读策略异构:派生/JoinMulti/`ReadLyricist`/comment/lyrics)**有意保留 switch**(语义不同不强合,同 CompareScoresDescending 否决先例)。39 characterization + 4 音频 round-trip + Workflow 3-lens 对抗验证等价。详见下方章节。
...
- 2026-07-01 **Tier C / ConfigDescriptorState 部分完成 `17c096f1`+`bed055a6`**（详见下方「ConfigDescriptorState」章节 + Tier C 清单）：`Id3v2FrameId`/`XiphFieldId`/`ApeFieldId` 三并行 field→标识符 switch（28 case）合成单张 `rawFieldVocabulary` 表（10 行×3 列）+ 3 查表方法（`field != null && TryGetValue` 守卫保原 `switch(null)→default→null`，`Dictionary.TryGetValue(null)` 抛 ANE 故守卫不可省）；`ReadFieldText` 读策略异构有意保留 switch。39 characterization（641→680）+ 4 音频 round-trip + Workflow 3-lens（映射 / null-comparer / 调用点-init）全 EQUIVALENT。Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / StateFieldInstance·PictureCompressionWorker 完成 `65b623c`**：`PictureCompressionWorker` 嵌套类 15 个纯转发方法（11 档固定梯度 `ResizeToNNNQualityMM` + 4 档 `ResizeToConfiguredLimitQualityMM`，各仅 `return resizeWorkingImageAndEncode(分辨率,质量)`）删除，替换为 2 张 `internal static readonly` 表（`fixedResolutionRetrySteps` `(int,long)[]` 11 元组、`configuredLimitRetryQualities` `long[]` 4 质量）；`CompressPictures` 组装点从硬编码 `new Func<bool>[11]/[4]` 方法组数组改 `Array.ConvertAll` 投影 lambda（`maxResolution==0` 走固定表、否则走配置表且 resolution 固定 `options.maxResolution`），净 −64 行。等价：①表 11+4 档顺序/值逐档对应原方法排列（重数 500×3/300×2 校验）；②`ConvertAll` lambda 形参每次独立无循环变量捕获陷阱、捕获 while 每迭代新建 `pictureCompressionWorker`；③配置表 lambda 调用时读 `options.maxResolution` 保求值时机；④返回 `Func<bool>[]` 类型/长度不变、`CompressPicture` 消费逐字节未动（git diff 仅 2 hunk）；⑤15 方法仅此组装点引用（全库 grep 无悬空）。图像压缩无 characterization（难单元测），靠数据流 + Workflow 3-lens（顺序值 / 闭包捕获-求值时机 / 数组类型-消费-引用，各从 git HEAD 独立恢复 BEFORE 逐槽核对）全 EQUIVALENT 0 反例 + 编译 + smoke（同 B4 先例）。680 passed、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / StateFieldInstance·3 FailureReporter 消息核提取 `ffbbef87`**：`SaveTagFailureReporter`/`UndoSaveTagFailureReporter`/`TagSaveFailureReporter` 三处 `ReportFailure` 的失败消息优先级逻辑（各 4-6 行）提取为 `internal static string ResolveFailureMessage(loadError, fallbackMessage) => !IsNullOrWhiteSpace(loadError) ? loadError : (fallbackMessage ?? Msg_SaveFail)`，三处改单行调用。⚠️ **核实并纠正 scout 误报**：line 146 原记"7 个 failure-reporter 类 + `AppendFileError(page,name,msg)`"，实为 **3 个 reporter、且无 `AppendFileError` 方法**（[[workflow-agent-fixture-trace-inconsistency]] 教训：scout 提议的方法名/数量须独立核实）。三原写法归约同一三元式（SaveTag/UndoSaveTag"fallback 先算再被非空 loadError 覆盖"、TagSave"先取 loadError 空则回退"，顺序反写、结果同）。副作用逐字节保留（`WriteSaveTagsLog`×2/`WriteClearTagsLog`、targetLog `batchContext.messageLog`/`taskContext.messageLog`/`Owner.errorLog`、filePath、`currentFile.Name`，git diff 仅 message 计算行变）。`GetLoadError()` 纯 getter（`return loadError;`），SaveTag 版 2→1 次调用 + `Msg_SaveFail` eager→lazy 均因无副作用/幂等非抛而不可观测。新增 4 characterization（`StateFieldInstancePureLogicCharacterization`，680→684；锁 loadError 优先级 + IsNullOrWhiteSpace 三态 null/empty/whitespace + `?? Msg_SaveFail` 回退链 + 实内容不 trim）。Workflow 3-lens（消息归约 / 求值-副作用 / 分派-可见性-调用点）全 EQUIVALENT 0 反例，与独立 git diff + 追踪交叉一致。684 passed、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / StateFieldInstance·BatchFileTaskContext 基类骨架 `4ec38a2`**：9 个批文件处理 context（RefreshItems/ConvertFilenameChinese/SaveTags/UndoSaveTags/UndoRename/ClearTags/DeleteFiles/SaveLrcFiles/ExtractCovers）各自重复的 4 公共字段（`cancellationSource`/`progressDialog`/`currentFile`/`processedCount`）+ 逐字节相同的 guarded `Cancel()`（`if (!IsCancellationRequested) Cancel();`）上移到新 `private class BatchFileTaskContext`，9 context 改继承。各 context 特有字段（items 数组/各类计数/`Page` log/owner 等）+ 各异 `UpdateProgress()`/业务方法留派生。净 −120 行（25+/145−）。⚠️ 核实纠正：实为 **9 个 context 非 ~11**；`LyricDownloadTaskContext`/`ReleaseYearSearchTaskContext` 因 `progressDialog` 为 `SimpleProgressDialog`（非 `ProgressDialog`）+ Cancel 无 guard、带 `CloseProgressDialog()` 而**不共享基类**（[[workflow-agent-fixture-trace-inconsistency]]）。等价：①字段上移基类、派生继承（同名同类型同默认值,无字段遮蔽——declaration count `cancellationSource`=3=基类+2未改、其余=1=仅基类）;②9 runner 全 `new X()`+post-construction 赋值（`ctx.field=..`）赋继承 public 字段,顺序不变;③`Cancel()` 经 `progressDialog.CancelRequested += ctx.Cancel` 方法组事件订阅（×9）绑继承基类 Cancel、读同一 `cancellationSource`;④UndoSave/UndoRename 的 4 公共字段与特有字段穿插,逐一精确删公共;⑤UndoSave/UndoRename 字段声明顺序穿插→基类分组,因无反射/序列化按 DeclaringType/metadata 访问而不可观测;⑥无反射/序列化触及这 10 类。运行时批处理路径无 characterization/smoke 覆盖,靠数据流 + Workflow 3-lens（字段上移语义 / Cancel上移分派 / 派生字段保留-引用完整）全 EQUIVALENT 0 反例 + 编译 + 684 characterization + smoke。**BatchFileTaskContext 仅基类骨架完成,runner 循环/取消/try-finally 脚手架统一待后续。** 684 passed、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / TextUtilities 4 纯方法 characterization `b265f1d1`**（纯测试,零产品改动）:补此前 0 覆盖的 `GetStringRespectingUtf16Bom`（UTF-16 BOM LE=FF FE/BE=FE FF 剥离 + HeaderName gate 非 utf-16 不剥 + length<2 fallthrough）、`TrimNonEmptyLines`（丢空行/逐行 Trim/`Environment.NewLine` 连接/null→null）、`DecodeBase64String`（UniCode 默认 + 显式编码名覆盖 + 非法 base64 catch→""）、`ContainsChinese`（`[一-龥]` 谓词）。5 例 golden-master,断言全 locale 无关确定性（byte→string 解码/regex/base64 round-trip）。689 passed（684→689）、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / StateFieldInstance·BuildClearTagsResultMessage `ff6cc72f`**:`StartClearTags` 完成段内联 `(string,bool)` 三分支消息构造（itemCount>1 拼 header+errorLog / success>0 裸 `Msg_CleartagsCompleted` / else 裸 errorLog+error flag）提为 `internal static BuildClearTagsResultMessage(itemCount,successCount,failedCount,processedCount,Page errorLog)`,callsite 收敛单次调用。**传 Page 引用（非预 ToString 的 string）**保 `errorLog.ToString()` 调用点/次数逐分支不变（1/0/1）、无 eager-eval 陷阱；5 参皆纯字段读、callsite 处 `await Task.Run` 后 UI 线程独占无并发写→一次性读=分支内读。4 characterization（689→693,锁三分支选择+bool 输出+拼接结构）+ Workflow 3-lens（eval-order/null-boundary/callsite-dataflow）全 EQUIVALENT 0 反例。693 passed、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / StateFieldInstance·InitializeComponent→Designer.cs partial 拆分 `8aebc2cb`**:god-form 的 `InitializeComponent()`（1566 行纯 designer 生成代码,原 6864-8429）搬至新建 `StateFieldInstance.Designer.cs`,主类 `internal class`→`internal partial class`；Dispose/components/控件字段留主文件（最小拆分,字段不动）。纯源码重新分区,编译器合并 partial 为同一类型→IL 相同,无逻辑/求值/字段布局改变；usings 全量复制（39 个）、方法体逐字节搬移。主文件 8497→6930 行（designer 噪音移出主逻辑文件）。运行时验证:Release exe 启动、主窗体实例化并运行 InitializeComponent、存活 5s（StartedAndStayedAlive=True）→ 无需 3-lens（无逻辑等价可推理,smoke 启动即最强验证）。693 passed、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / Serialization 编码 + JavaScriptStringEncode characterization `66b2349b`**（纯测试,零产品改动）:新建 `SerializationEncodingCharacterization`（`AsciiTokenClassifier.IsAsciiLetterLike`/`IsAsciiDigitLike` ASCII+全角范围、`TagTextEncoding.NormalizeEncodingName` 5 显式别名、`TranscodeText` ASCII/Latin1 round-trip）+ `UtilityMapping` 追加 `JavaScriptStringEncode` 4 例（`"`→`\"`、`<>`→unicode escape、`\n`→`\n`、identity）。断言全 locale 无关、确定性;probe-first ZERO FAIL（8 例首次命中）。701 passed（693→701）、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / ConfigDescriptorState 2 public static 纯方法 characterization `15f8526e`**（纯测试）:补此前 0 覆盖的 `GetGenreNameByIndex`（TagLib.Genres.Audio + bounds guard,(0)=Blues/(1)=Classic Rock、越界→""）+ `SupportedPictureMimeTypes`（固定 3 项 image/jpeg,png,gif）。probe-first ZERO FAIL（2 例）。703 passed（701→703）、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / StateFieldInstance·TruncateLyricsOrCommentDisplayValue `3b7219a4`**:`AddLoadedFilesToListView` 与 `UpdateListViewItemValues` 两列值填充循环内逐字节相同的 lyrics/comment >20 截断内联块（`(name=="lyrics"||"comment") && Length>20 → Substring(0,20).Trim()`）提取为 `internal static`,两 callsite 收敛单调用。`column.Name`（`ColumnHeaderInfo` get-only 纯 getter）求值由内联 `||` 短路 1~2 次变 callsite 无条件 1 次,因无副作用/幂等/非抛不可观测;未命中列原值返回。1 例 characterization（6 断言:>20 截断、余列原样、Length==20 边界不截、cut@20 后 Trim 次序）+ Workflow 3-lens（eval-count/dedup-equivalence/null-boundary）全 EQUIVALENT。704 passed（703→704）、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / AutoMatchTagsDialog 3 分类谓词上提 `7e457429`**:`AutoMatchWorker`（private nested）内 3 个 private static 纯谓词上提为外层 `AutoMatchTagsDialog` 的 internal static（仅可见性 private→internal + 移到外层类,零逻辑改动,嵌套调用点按简单名/方法组仍解析到外层,编译 EXIT=0）经 IVT 可测——`IsSameTrackMetadata`（Title+Artist+Album ordinal 全等）/`IsTextTagMatchKey`（排除 cover/lyrics）/`IsYearFieldName`（精确 "year"）。手法同已落地文本标签 gate 三兄弟。3 例 characterization（ordinal locale 无关、大小写敏感 + null 边界）。707 passed（704→707）、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / StateFieldInstance·Build{DeleteFiles,ExtractCovers}ResultMessage `b79d07ca`**:`DeleteFilesTaskContext`/`ExtractCoversTaskContext` 各 `ShowCompletionResult` 内逐字节同构的三路（单项成功 ShowInfo / 单项全败 ShowError / 多项 ShowInfo+Format）消息构造提取为外层两个 `internal static (string,bool)`（同 `BuildClearTagsResultMessage`;Item1=消息、Item2=isError 仅单项全败 true）,两处收敛为 `var r=Build(...); if(r.Item2) ShowError(r.Item1); else ShowInfo(r.Item1);`（DeleteFiles 保留派发后无条件 `PerformClick`、ExtractCovers 无）。errorLog 传 Page 引用保 `ToString` 求值次数（0/1/1）;`string.Format` 格式串/实参序、Resources 键、`<=1` vs `>1` 阈值不变。8 例 characterization（各 4）+ Workflow 3-lens（show-mapping/eval-count/tail-controlflow）全 EQUIVALENT（eval-count lens 核实 eager-arg-eval 为纯 int 字段读 benign 变体、零可观测差异）。715 passed（707→715）、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / StateFieldInstance·CompareColumnText `b71b6793`**:`ListViewItemNaturalComparer.Compare`（private nested `IComparer`）内排序比较逻辑（自然序列名判定 trackstr/discstr + `SortOrder` 方向分派）提取为外层 `internal static int CompareColumnText(leftText,rightText,columnName,sortOrder)`,Compare 保留 FileRow/列索引/`configuredColumnHeaders[i].Name` 解引用后调之（因 comparer private nested、测试即便 IVT 不可达其成员,纯核置外层方经 IVT 可测;nested 调外层 static 简单名解析,AutoMatch 先例）。switch/useNaturalSort 原样搬入、仅 `sortSetting.SortOrder` 形参化;deref 全留 Compare（NRE 时机不变）;`sortSetting.SortOrder` 求值 1 次不变、无 eager-eval trap、无分支重排。7 例 characterization（以 `CompareNaturalText`/`string.Compare` 自身作 golden → culture-safe,锁路径/方向/参数转发）+ Workflow 3-lens（branch-equivalence/eval-count/resolution-signature）全 EQUIVALENT。722 passed（715→722）、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / 尾批 5 纯逻辑 characterization + 4 可见性提升 `0ad80c6f`**（纯测试 + 纯可见性）:scout（4-agent 穷尽剩余 Tier C）甄别的低风险候选——4 处 `private static`→`internal static`（body 逐字节不变:`ConfigDescriptorState.StringTypeName`/`EncodeByStringType`、`EditableListView.CompareSortableText`、`CustomColumnsDialog.CompareColumnDisplayOrder`）+ `Page`（已 IVT 可达）直接 characterize。5 例（`Page` 有界日志封顶 20 + 幂等 "…"、`StringTypeName` 5 分支 + UTF8 default、`EncodeByStringType` null 归一/`BitConverter` 比字节/UTF16·LE·unknown→Unicode LE、`CompareSortableText` 数值>日期>字典序、`CompareColumnDisplayOrder` 有效序升序/tie shown 后 hidden）。probe-first ZERO FAIL。可见性提升不破坏 caller、纯 characterization → 无需 3-lens。727 passed（722→727）、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / StateFieldInstance 3 纯核提取 `302bfa65`**:`IsInvalidRenameFileName`（PerformInPlaceRename 就地改名非法文件名谓词）/`ResolveSelectedFilterValue`（GetSelectedFilterValue 过滤值归一,comment 截断哨兵 "Y\tT"/空白/lyrics/原值 3 决策）/`IsNumberedTagFieldValueValid`（ValidateNumberedTagField track/disc 数字字段接受谓词,`||` 短路链）提取 internal static,原方法委托。3 characterization——`ResolveSelectedFilterValue` 保留 comment+null latent NRE（try/catch 断言,harness 无 Check.Throws）;`IsInvalidRenameFileName` 锁定 .NET FW `Path.GetFileName` 对 InvalidPathChars（`<>|"`）抛 ArgumentException 边界（.NET Core 改为不抛;原 inline 同在 try 块经同一 catch→ShowRenameError,与 Msg_InvalidFile 路径殊途同归弹错拒绝）。eager-eval 安全（CommentFullLength/RequestedFileName public 纯字段读、ParseLeadingNumber 纯 + null 被 IsNullOrWhiteSpace 短路永不入 Regex）。probe-first 抓 1 断言错（.NET Core 直觉误判 GetFileName 不抛）修正后 3-lens 全 EQUIVALENT。730 passed（727→730）、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / AutoMatchTagsDialog 2 纯核提取 `5da37a0b`**:`BuildAutoMatchCompletionResult`（StartAutoMatchTags inline if-else + 私有 CreateBatchAutoMatchResult 合并 4 路径完成文案,与 FilenameRelatedBatchDialog.BuildBatchCompletionResult 同构）/`IsOnlyWriteFileModeSelected`（加 static 重载纯核 + 原无参实例转 wrapper,caller StateFieldInstance:6311 无参实例绑定实例重载零改动,锁空集 All 陷阱 `.Any()` 守卫不可省）。7 characterization（6 + 1）。eager-eval 安全:successCount 等 volatile int,worker join（await Task.Run 完成建内存屏障、ProcessParallelQueue 队列耗尽无并发写）后值已定,无条件读全 4 无副作用（volatile 读原子/不抛/不写,丢弃的读仅 acquire 栅栏）;selectedMatchConditions readonly 纯字段 getter。3-lens 全 EQUIVALENT。737 passed（730→737）、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / 3 跨文件纯核提取 `097fe1e0`**:`ConfigDescriptorState.BuildFileExtension`（文件扩展名归一,private 无参→internal static filePath,Path.GetExtension 去点 ToUpperInvariant/null·无扩展名→""）/`TextBoxFindReplaceController.ReplaceAllMatches`（全量非重叠替换,SearchText 纯 auto-property/GetStringComparison 纯提为参数,循环内每迭代 N 次读归约调用点 1 次——ReplaceAll 同步无重入 SearchText/MatchCase 恒定）/`LyricSearchDialog.ResolveLyricIconIndex`（歌词图标 0/1,调用点传预计算 `lyricUrl==null && HasDownloadableLyric()` 保短路在实参内、非拆两实参,HasDownloadableLyric 仅 lyricUrl==null 时求值——非 eager-eval 陷阱）。3 characterization。3-lens 全 EQUIVALENT。740 passed（737→740）、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / StateFieldInstance·ResolvePictureCompressionLimits `c052031`**:CompressPictures prologue 图片压缩上限解算（还原模式固定 10MB/无分辨率限制/AUTO;否则取 Settings 配置）提取 internal static 返回 `(long MaxByteLength, int MaxResolution, string FormatMode)`,Settings 值传参（getter 纯 `(T)this[...]`,无 override/事件/自定义索引器）。**锁定 sizeLimitKB*1024 的 int 乘法溢出边界**:三元两分支均 int（10240000 literal + int*1024）自然类型 int 赋 long（扩展在 int 域乘法后）;csproj `CheckForOverflowUnderflow=False`（unchecked 回绕）溢出成负→`<=0L`→long.MaxValue;严禁提升 long 乘法。1 characterization（restore/config 双路 + 0/负归一 + 2097151*1024=2147482624 不溢出 / 2097152*1024 溢出成 -2147483648→MaxValue 边界）。3-lens（int-multiply-overflow/eager-settings-read/ternary-branch-tuple）全 EQUIVALENT。741 passed（740→741）、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **Tier C / StateFieldInstance·ClampWindowToWorkingArea `414275ee`**（中风险,最严格对抗验证）:OnLoad 非最大化分支窗口位置 clamp 到工作区顺序几何（右/下越界回拉→整体出界归零）提取 internal static `ClampWindowToWorkingArea(location, size, workingArea)` 纯几何,调用点保留 base.Size clamp、收敛为单次 `base.Location = Clamp(…)` 写。BEFORE 0~3 次条件写 base.Location（顺序依赖:后 if 读前 if 写值）→ AFTER 局部 Point 顺序读写模拟 + 1 次无条件写。等价三支柱:①数据流 bit-identical（含 order-matters 用例）;②WinForms `Control.SetBoundsCore` 值比较守卫使相同值 no-op（无 clamp 时写回原值≡零写）;③无观察者（grep 无 LocationChanged/Move 订阅/override,SizeChanged handler 不碰 Location,OnLoad 时窗体未 Show,后续仅关闭读终值,OS 不夹顶层窗口坐标故中间读不分叉）。1 characterization（纯几何 workingArea 传参 7 断言）+ Workflow 3-lens（几何数据流/WinForms-setter 语义/无观察者,对抗性寻反例失败）全 EQUIVALENT。742 passed（741→742）、Debug+Release 0 warn、smoke 通过。
- 2026-07-01 **本轮 Tier C pure-core 层收敛评估**:自 `b265f1d1`（722）起本轮共 7 批（尾批可见性 `0ad80c6f` + `302bfa65`/`5da37a0b`/`097fe1e0`/`c052031`/`414275ee`）完成 StateFieldInstance（`IsInvalidRenameFileName`/`ResolveSelectedFilterValue`/`IsNumberedTagFieldValueValid`/`ResolvePictureCompressionLimits`/`ClampWindowToWorkingArea` + 尾批 3 可见性）、AutoMatchTagsDialog（`BuildAutoMatchCompletionResult`/`IsOnlyWriteFileModeSelected`）、ConfigDescriptorState（`BuildFileExtension`）、TextBoxFindReplaceController（`ReplaceAllMatches`）、LyricSearchDialog（`ResolveLyricIconIndex`）等 **15 处纯核提取 + characterization**,722→742（+20,总 742 passed）,每批 probe-first ZERO FAIL + 敏感提取 Workflow 3-lens 全 EQUIVALENT（含中风险 `ClampWindowToWorkingArea` 的多写→单写经数据流/WinForms-setter/无观察者三 lens 对抗验证）。scout（4-agent 穷尽扫描）判定低/中低风险 pure-core 候选**已枯竭**,连原暂缓的 `ClampWindowToWorkingArea` 亦经严格对抗验证纳入。剩余仅 `CoverPreviewController`/VirtualMode 选择模型/track-search 三元组/封面缩略图子系统（均已评估为 UI 耦合 / un-constructable context,不宜 pure-core 提取）。**Tier C 低/中低风险 golden-master 锁定层本轮完全收官。**
