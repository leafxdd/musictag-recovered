// QRC decryption is adapted from jitwxs/163MusicLyrics (Apache-2.0).
// Source: https://github.com/jitwxs/163MusicLyrics/blob/master/cross-platform/MusicLyricApp/Core/Utils/QQMusicearchUtils.cs
using System;
using System.IO;
using System.Text;
...
internal static class QqQrcDecoder
{
	...
	internal static string DecryptLyrics(string encryptedLyrics)
	{
		...
		{
			var temp = new byte[8];
			QqDesHelper.TripleDESCrypt(encryptedTextByte[i..], temp, schedule);
			for (int j = 0; j < 8; j++)
			{
				data[i + j] = temp[j];
			}
	...
	internal static bool LooksEncryptedLyrics(string value)
	{
	...
	internal static string ConvertToLineLyric(string decryptedLyrics, bool useThreeDigitMilliseconds)
	{
		string lyricText = ExtractLyricContent(decryptedLyrics);
		if (string.IsNullOrWhiteSpace(lyricText))
		{
			return "";
		}

		MatchCollection timestampMatches = QrcLineTimestampRegex.Matches(lyricText);
		...
		int firstTimestampIndex = timestampMatches[0].Index;
		foreach (Match metadataMatch in MetadataTagRegex.Matches(lyricText.Substring(0, firstTimestampIndex)))
		{
			lyricBuilder.Append(metadataMatch.Value);
			lyricBuilder.Append('\n');
		}
	...
	private static string ExtractLyricContent(string decryptedLyrics)
	{
		if (string.IsNullOrWhiteSpace(decryptedLyrics) || !decryptedLyrics.Contains("<", StringComparison.Ordinal))
		{
		...
		try
		{
			XDocument document = XDocument.Parse(decryptedLyrics);
			foreach (XElement element in document.Descendants())
			{
				XAttribute lyricContent = element.Attribute("LyricContent");
				if (lyricContent != null && !string.IsNullOrWhiteSpace(lyricContent.Value))
				{
					return lyricContent.Value;
				}
			}
		}
		catch (Exception)
	...
	private static byte[] SharpZipLibDecompress(byte[] data)
	{
		using var compressed = new MemoryStream(data);
		using var decompressed = new MemoryStream();
		using var inputStream = new ZLibStream(compressed, CompressionMode.Decompress);
...
internal static class QqDesHelper
{
	internal static readonly uint ENCRYPT = 1;
	internal static readonly uint DECRYPT = 0;
